...
void* setpoint_manager_func(__attribute__ ((unused)) void* ptr){
	double tmp;
...
int start_setpoint_manager(setpoint_t* setpoint, user_input_t* user_input, \
	...
	cs = cstate;
	set = settings;
	if(rc_pthread_create(&pthread, setpoint_manager_func, NULL, SCHED_FIFO, SETPOINT_MANAGER_PRI)<0){
		fprintf(stderr,"ERROR in start_setpoint_manager, failed to start thread\n");
		return -1;
	}
	rc_usleep(1000);
	return 0;
...
int join_setpoint_manager_thread(){
	...
	int ret = rc_pthread_timed_join(pthread,NULL,SETPOINT_MANAGER_TOUT);
	if(ret==1) fprintf(stderr,"WARNING: setpoint_manager_thread exit timeout\n");
	else if(ret==-1) fprintf(stderr,"ERROR: failed to joing setpoint_manager thread\n");
	return ret;
}
